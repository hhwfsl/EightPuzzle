...
public partial class MainWindow : Window
{
    ...
    public required List<Node> FlatMapL { get; set; }//数据绑定UniformGrid的九个按钮对应Node(当前)
    public required List<Node> FlatMapR { get; set; }//数据绑定UniformGrid的九个按钮对应Node(目标)
    Node Blank = new Node();//空格
    int stepCount = 0;//完成所用步数
    public MainWindow()
    {
        InitializeComponent();
    ...
    {
        MessageBox.Show("还没做","信息",MessageBoxButton.OK,MessageBoxImage.Information);
    }
}